...
class _AccountRecoveryScreenState extends State<AccountRecoveryScreen> {
  ...
  void initState() {
    ...
    DeliveryMan? deliveryMan = Get.find<AuthController>().deliveryMan;

    if(deliveryMan?.imageFullUrl != null) {
      authController.clearDmImage();
      authController.saveDmImage(deliveryMan!.imageFullUrl!);
    }

    _countryDialCode = CountryCode.fromCountryCode(Get.find<SplashController>().configModel!.country!).dialCode;
    _splitPhone(deliveryMan?.phone);

    _fNameController.text = deliveryMan?.fName ?? '';
    _lNameController.text = deliveryMan?.lName ?? '';
    _emailController.text = deliveryMan?.email ?? '';
    _addressController.text = deliveryMan?.dmAddress ?? '';
    _identityNumberController.text = deliveryMan?.identityNumber ?? '';

    if(deliveryMan?.dmAddressProofFullUrl != null && deliveryMan!.dmAddressProofFullUrl!.isNotEmpty) {
      authController.clearProofAddressImage();
      for(String image in deliveryMan.dmAddressProofFullUrl!) {
        authController.saveProofAddressImage(image);
      }
    }

    if(deliveryMan?.identityImageFullUrl != null && deliveryMan!.identityImageFullUrl!.isNotEmpty) {
      authController.clearIdentityImage();
      for(String image in deliveryMan.identityImageFullUrl!) {
        authController.saveIdentityImage(image);
      }
    }
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      child: Scaffold(
        appBar: CustomAppBarWidget(title: 'Account Recovery'.tr, onBackPressed: (){
            ...
            return Column(children: [
              ...
              Container(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Text(authController.deliveryMan?.reason ?? 'No reason provided',
                    ...
                    child: Column(mainAxisSize: MainAxisSize.min, mainAxisAlignment: MainAxisAlignment.start, children: [
                      ...
                      authController.vehicleIds != null ? Container(
                        ...
                        child: CustomDropdown<int>(
                          ...
                          items: vehicleList,
                          child: Text(authController.vehicles?[0].type ?? 'select_vehicle_type'.tr),
                        ),
  ...
  void _addDeliveryMan(AuthController authController, AddressController addressController) async {
    ...
    if(authController.vehicleIndex!-1 == -1) {
      ...
      authController.registerDeliveryMan(DeliveryManBodyModel(
        ...
        dmAddress: address, dmPreviousId: authController.deliveryMan?.id, isForResubmit: 1,
      ));
    }
  }
}